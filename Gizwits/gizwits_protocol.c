...
int32_t gizPutData(uint8_t *buf, uint32_t len)
{
    ...
    if(count != len)
    {
        GIZWITS_LOG("ERR: Failed to rbWrite. count=%d, len=%d\r\r\n", count, len);
        return -1;
    }
...
static int8_t gizProtocolHeadInit(protocolHead_t *head)
{
    if(NULL == head)
    {
        GIZWITS_LOG("ERR: gizProtocolHeadInit head is empty \r\r\n");
        return -1;
    }
...
static int8_t gizProtocolWaitAck(uint8_t *gizdata, uint32_t len)
{
    if(NULL == gizdata)
    {
        GIZWITS_LOG("ERR: data is empty \r\r\n");
        return -1;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    {
        GIZWITS_LOG("gizDataPoint2Event Error , Illegal Param\r\r\n");
        return -1;
    }
    ...
    if(sizeof(issuedData->attrFlags) > 1)
    {
        ...
        {
            GIZWITS_LOG("gizByteOrderExchange Error\r\r\n");
            return -1;
        }
    }
    
    if(0x01 == issuedData->attrFlags.flagGIVE_TISSUE)
    {
        info->event[info->num] = EVENT_GIVE_TISSUE;
        info->num++;
        dataPoints->valueGIVE_TISSUE = gizStandardDecompressionValue(GIVE_TISSUE_BYTEOFFSET,GIVE_TISSUE_BITOFFSET,GIVE_TISSUE_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagCURRENT_LENGTH)
    {
        info->event[info->num] = EVENT_CURRENT_LENGTH;
        info->num++;
        dataPoints->valueCURRENT_LENGTH = gizStandardDecompressionValue(CURRENT_LENGTH_BYTEOFFSET,CURRENT_LENGTH_BITOFFSET,CURRENT_LENGTH_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    {
        GIZWITS_LOG("gizCheckReport Error , Illegal Param\r\r\n");
        return -1;
    }
    if(last->valueGIVE_TISSUE != cur->valueGIVE_TISSUE)
    {
        GIZWITS_LOG("valueGIVE_TISSUE Changed\r\r\n");
        ret = 1;
    }
    if(last->valueCURRENT_LENGTH != cur->valueCURRENT_LENGTH)
    {
        GIZWITS_LOG("valueCURRENT_LENGTH Changed\r\r\n");
        ret = 1;
    }
    if(last->valueFULL_TISSUE != cur->valueFULL_TISSUE)
    {
        GIZWITS_LOG("valueFULL_TISSUE Changed\r\r\n");
        ret = 1;
    }
    if(last->valueNO_TISSUE != cur->valueNO_TISSUE)
    {
        GIZWITS_LOG("valueNO_TISSUE Changed\r\r\n");
        ret = 1;
    }
    if(last->valueERR_JAM != cur->valueERR_JAM)
    {
        GIZWITS_LOG("valueERR_JAM Changed\r\r\n");
        ret = 1;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    {
        GIZWITS_LOG("gizDataPoints2ReportData Error , Illegal Param\r\r\n");
        return -1;
    }
    ...
    gizStandardCompressValue(GIVE_TISSUE_BYTEOFFSET,GIVE_TISSUE_BITOFFSET,GIVE_TISSUE_LEN,(uint8_t *)devStatusPtr,dataPoints->valueGIVE_TISSUE);
    if(dataPoints->valueCURRENT_LENGTH >= CURRENT_LENGTH_VALUE_MAX)
    {
        GIZWITS_LOG("[ERROR] valueCURRENT_LENGTH Error , Illegal Overstep\r\r\n");
        return -1;
    }
    else
    {
        gizStandardCompressValue(CURRENT_LENGTH_BYTEOFFSET,CURRENT_LENGTH_BITOFFSET,CURRENT_LENGTH_LEN,(uint8_t *)devStatusPtr,dataPoints->valueCURRENT_LENGTH);  
    }
    gizStandardCompressValue(FULL_TISSUE_BYTEOFFSET,FULL_TISSUE_BITOFFSET,FULL_TISSUE_LEN,(uint8_t *)devStatusPtr,dataPoints->valueFULL_TISSUE);
    gizStandardCompressValue(NO_TISSUE_BYTEOFFSET,NO_TISSUE_BITOFFSET,NO_TISSUE_LEN,(uint8_t *)devStatusPtr,dataPoints->valueNO_TISSUE);
    gizStandardCompressValue(ERR_JAM_BYTEOFFSET,ERR_JAM_BITOFFSET,ERR_JAM_LEN,(uint8_t *)devStatusPtr,dataPoints->valueERR_JAM);
...
static int8_t gizProtocolIssuedProcess(char *did, uint8_t *inData, uint32_t inLen, uint8_t *outData, uint32_t *outLen)
{
    ...
    {
        GIZWITS_LOG("gizProtocolIssuedProcess Error , Illegal Param\r\r\n");
        return -1;
    }
...
static int32_t gizProtocolIssuedDataAck(protocolHead_t *head, uint8_t *gizdata, uint32_t len, uint8_t proFlag)
{
    ...
    if(NULL == gizdata)
    {
        GIZWITS_LOG("[ERR]  data Is Null \r\r\n");
        return -1;
    }
    ...
    GIZWITS_LOG("len = %d , sDidLen = %d ,data_len = %d\r\r\n", len,sDidLen,data_len);
    *pTxBuf ++= 0xFF;
    *pTxBuf ++= 0xFF;
    ...
    *pTxBuf ++= proFlag;
    ...
    if(ret < 0)
    {
        GIZWITS_LOG("uart write error %d \r\r\n", ret);
        return -2;
    }
...
static int32_t gizReportData(uint8_t action, uint8_t *gizdata, uint32_t len)
{
    ...
    if(NULL == gizdata)
    {
        GIZWITS_LOG("gizReportData Error , Illegal Param\r\r\n");
        return -1;
    }
    ...
    if(ret < 0)
    {
        GIZWITS_LOG("ERR: uart write error %d \r\r\n", ret);
        return -2;
    }
...
static void gizDevReportPolicy(dataPoint_t *currentData)
{
    ...
    {
        GIZWITS_LOG("changed, report data\r\r\n");
    ...
    {
        GIZWITS_LOG("Info: 600S report data\r\r\n");
        if(0 == gizDataPoints2ReportData(currentData,&gizwitsProtocol.reportData.devStatus))
        {
            gizReportData(ACTION_REPORT_DEV_STATUS, (uint8_t *)&gizwitsProtocol.reportData.devStatus, sizeof(devStatus_t));
        }       
...
static int8_t gizProtocolGetOnePacket(rb_t *rb, uint8_t *gizdata, uint16_t *len)
{
    ...
    {
        GIZWITS_LOG("gizProtocolGetOnePacket Error , Illegal Param\r\r\n");
        return -1;
    }
...
static int32_t gizProtocolCommonAck(protocolHead_t *head)
{
    ...
    if(NULL == head)
    {
        GIZWITS_LOG("ERR: gizProtocolCommonAck data is empty \r\r\n");
        return -1;
    }
...
static int32_t gizProtocolGetDeviceInfo(protocolHead_t * head)
{
    ...
    if(NULL == head)
    {
        GIZWITS_LOG("gizProtocolGetDeviceInfo Error , Illegal Param\r\r\n");
        return -1;
    }
...
static int32_t gizProtocolErrorCmd(protocolHead_t *head,errorPacketsType_t errno)
{
    ...
    if(NULL == head)
    {
        GIZWITS_LOG("gizProtocolErrorCmd Error , Illegal Param\r\r\n");
        return -1;
    }
...
{  
    ...
    if(NULL == head)
    {
        GIZWITS_LOG("ERR: NTP is empty \r\r\n");
        return -1;
    }
...
static int8_t gizProtocolModuleStatus(protocolWifiStatus_t *status)
{
    ...
    if(NULL == status)
    {
        GIZWITS_LOG("gizProtocolModuleStatus Error , Illegal Param\r\r\n");
        return -1;
    }
